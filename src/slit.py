...
class Slit(object):
    ...
    @staticmethod
    def determine_slit_rows(data, window_size=39, order=4, distance=100, verbose=True, detection_level=1000):
        '''
        ...
        if verbose:
            ...
            shape = data.shape
            for peak in peaks:
                plt.plot([0,shape[1]],[peak,peak], color='yellow')
            plt.show()
